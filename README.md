...
- ✅ **適用時期の明示**（EPOS/TWIST などの時期依存ルール）
- ✅ **予後コンセンサス対象かどうかの区別**

### 3. **文献コンセンサス分析**

複数の**予後ルール**を統合し、「どれだけの文献が歩行自立を示唆しているか」をスコア化・可視化します。

※ Perry歩行速度分類は、現時点の歩行レベル分類として表示しますが、将来予後のコンセンサス集計には含めません。
...
発症からの日数に応じて、EPOS（3日以内）とTWIST（7日以内）の適用可否が入力フォーム上に表示されます。
...
- **補足**: 現時点の歩行レベルの参考分類として表示し、文献コンセンサス分析には含めません
...
1. **基本情報**: 年齢、性別、発症からの日数、病型
   発症からの日数を入力すると、EPOS/TWIST の適用可否がその場で表示されます。
...
#### A. 文献コンセンサス分析
- **スコア**: 自立寄りの予後ルールの割合（0-100%）
- **トーン**: 
  - 70%以上: 自立寄りのコンセンサス（緑）
  - 40-70%: 拮抗（黄）
  - 40%未満: 困難寄りのコンセンサス（赤）
- **集計対象外**: Perry歩行速度分類は参考表示であり、コンセンサスには含まれません

#### B. 個別ルール結果
各ルールカードには以下が表示されます:
- ルール名とエビデンスレベル
- 予測結果（色分け: 緑=自立寄り、赤=困難寄り、青=参考表示）
- 詳細情報（スコア、カットオフ、精度指標）
- 時期依存ルールの適用範囲
